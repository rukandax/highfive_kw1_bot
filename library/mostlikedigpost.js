...
async function getMostLikedIgPost(ctx) {
  ...
  if (ctx.message.text.includes("/mostlikedigpost@highfive_kw1_bot")) {
    username = ctx.message.text
      .replace("/mostlikedigpost@highfive_kw1_bot", "")
      .trim();
  } else {
  ...
  if (username.length <= 0) {
    return ctx
      .replyWithHTML("Username instagram nya siapa ?", {
  ...
  ctx
    .reply("Sebentar ya, aku liat-liat dulu..", {
      reply_to_message_id: ctx.message.message_id
    })
    .catch(err => {
      console.log(err);
  ...
  try {
    ...
    const payload = await page.evaluate(() => {
      ...
      if (selectedCaption.length > 1024) {
        selectedCaption =
          selectedCaption.substr(0, 1001) + " ... (caption too long)";
      }
